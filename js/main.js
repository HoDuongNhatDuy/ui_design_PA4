$(document).ready(function () {
	...
	$('#upload-file-pet').on('change', function (event) {
		...
		reader.onload = function (e) {
			...
			var divimg = `
                        ...
                        <div class="remove-image-pet"></div>
                    </div>
            			`;
	...
	$('.div-choose-image-pet').on('click', '.remove-image-pet', function () {
		$(this).closest(".image-pet").remove();
		addclassnoneimage();
	});

	function addclassnoneimage() {
		if ($('.div-choose-image-pet .image-pet').length === 0) {
			if (!$('.btn .add-image-pet').hasClass('none-image-choose'))
				$('.btn .add-image-pet').addClass('none-image-choose');
		} else {
			if ($('.btn .add-image-pet').hasClass('none-image-choose'))
				$('.btn .add-image-pet').removeClass('none-image-choose');
		}
	}
